...
普通模式继续保持“吃 / 弃 / 后置 + 轮末三选一”的轻量流程；完成对局后会逐步解锁备料、商店、条约商店、无尽与高难模式。解锁只增加玩法选择，不提供局外数值成长。
...
初始牌组使用苹果；苹果自带水果连击 +1，梨在连击达到 3 或以上时额外提供刷新标记。对局会在操作、结算、刷新、选牌和商店操作后自动保存，可随时回到主界面继续。
...
- 任意模式通关 1 次：解锁商店、无尽与高难模式。商店模式恢复经典经济牌、经济道具和买牌 / 扩容 / 删牌三角取舍。
- 商店模式通关 1 次：解锁条约商店。每轮选择一条经济合约；12 秒内清盘 +1 金币，8 秒内清盘 +2 金币。
- 无尽模式：道具可重复获得；每 5 轮赠送扩容和删牌标记，餐盘最多 16 张；累计 1,000,000 分通关并获得主界面 `GOD` 标记。

v0.20–v0.23 的进行中对局会迁移到 v0.24 状态，保留牌组、轮次与分数并补充模式字段。
...
- `js/main.js`：普通、备料、商店、条约、无尽与高难流程。
